...
for i in range(2,3):
    ...
    t2 = S.dot(sep).dot(tree_dist(Tree.from_penn('(D a)'))).dot(sep.T).dot(D.T).dot(D).dot(sep).dot(Phi).dot(sep.T).dot(E.T).dot(index3.T).dot(index1.T).dot(index1).dot(index3).dot(E).dot(sep).dot(tree_dist(Tree.from_penn('(E b)')))
    #Pd = cyk_dist(G,w)
    #print Pd.dot(P_1.T)
    print t1.dot(t2.T)
    #x = circulant(v('sdfg'))
