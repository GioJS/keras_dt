...
for i in range(2,3):
    ...
    P = init(w)
    #print P_dist
    P = preterminals(P, G, w)    #Pa = C.dot(sc(v('D').dot(sc(v('Sep'))).dot(tree_dist(Tree.from_penn('(D a)'))).dot(invsc(v('Sep'))).dot(invsc(v('D')))).dot(Ra).dot(sc(v('E')).dot(sc(v('Sep'))).dot(tree_dist(Tree.from_penn('(E b)'))).dot(invsc(v('Sep'))).dot(invsc(v('E'))))).dot(C.T)
    Pa = C.dot(invsc(v(str(1))).dot(invsc(v(str(1)))).dot(P).dot(Ra).dot(invsc(v(str(3)))).dot(invsc(v(str(1)))).dot(P)).dot(C.T)
