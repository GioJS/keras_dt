...
def preterminals(P,G,w):
    ...
    #print R
    for i in range(len(w)):
        s = (sc(v('1')).dot(sc(v(str(i)))).dot(R).dot(invsc(v('Sep'))).dot(invsc(v(w[i]))).dot(invsc(v(str(i)))).dot(invsc(v('0'))).dot(P))
        #print s
        P = P + s
    return P
#binary rules
...
for i in range(2,3):
    ...
    from trees import *
    Pd = Pd.dot(sc(v('D'))).dot(sc(v('Sep'))).dot(circulant(v('a')).T).dot(invsc(tree_dist(parser.get_tree(parser.C[0,0][0])))).invsc(v('Sep'))
        # # print Pd
    Dp = test_P(parser,w)
